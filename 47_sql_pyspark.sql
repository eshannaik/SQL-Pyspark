-- CREATE TABLE hotel_reviews_airbnb_47 (additional_number_of_scoring BIGINT, average_score FLOAT, days_since_review NVARCHAR(255), hotel_address NVARCHAR(255), hotel_name NVARCHAR(255), lat FLOAT, lng FLOAT, negative_review NVARCHAR(1096), 
-- positive_review NVARCHAR(1096), review_date DATETIME, review_total_negative_word_counts BIGINT, review_total_positive_word_counts BIGINT, reviewer_nationality NVARCHAR(255), reviewer_score FLOAT, tags NVARCHAR(1096), total_number_of_reviews BIGINT, 
-- total_number_of_reviews_reviewer_has_given BIGINT);

-- INSERT INTO hotel_reviews_airbnb_47 VALUES
...
SELECT hotel_name,COUNT(*) as 'neg_count'
FROM hotel_reviews_airbnb_47
...
hotel_reviews_airbnb_47.filter(col("negative_review") != "No Negative")\
	.groupBy("hotel_name")\
	.agg(count("*").alias("neg_count"))\
    .select("hotel_name","neg_count")
    .orderBy(col("neg_count").desc())\
	.limit(2).show()
